...
const JSTableDefaultConfig = {
    ...
    labels: {
        placeholder: "Search...",
        perPage: "{select} tickets per page",
        noRows: "No entries found",
        info: "Showing {start} to {end} of {rows} tickets",
        loading: "Loading...",
        infoFiltered:
...
class JSTable {
    ...
    _build() {
        ...
        let container = this.wrapper.querySelector(
            "." + options.classes.container
        );
    ...
    getDataCountTotal() {
        if (this.config.serverSide) {
            return this.config.deferLoading !== null
                ? this.config.deferLoading
                : this.dataCount;
        }
    ...
    async search(query) {
        ...
        if (!this.config.serverSide) {
            this.table.dataRows.forEach(function (row) {
                ...
                var match = that.searchQuery
                    .split(" ")
                    .reduce(function (bool, word) {
                        var hasMatch = false;
            ...
            this.wrapper.addEventListener("keyup", function (e) {
                if (
                    ...
                    if (that.config.addQueryParams) {
                        const url = new URL(window.location.href);
                        url.searchParams.set(
                            that.config.queryParams.search,
                            e.target.value
                        );
                        window.history.replaceState(null, null, url);
                    }
            ...
            this.config.columns.forEach(function (columnsDefinition) {
                ...
                columnsDefinition.select.forEach(function (column) {
                    ...
                    if (columnsDefinition.hasOwnProperty("sortable")) {
                        ...
                        if (sortable) {
                            tableHeaderCell.addClass(
                                that.config.classes.sorter
                            );
    ...
    getFormatted(columnRenderers, rowAttributesCreator = null) {
        ...
        this.getCells().forEach(function (cell, idx) {
            ...
            if (columnRenderers.hasOwnProperty(idx)) {
                td.innerHTML = columnRenderers[idx].call(
                    that,
                    cell.getElement(),
                    idx
                );
...
class JSTablePager {
    ...
    render() {
        ...
        if (pages > 1) {
            ...
            if (options.nextPrev) {
                ul.appendChild(
                    this.createItem("pager", prev, options.prevText)
                );
            }
            ...
            if (options.nextPrev) {
                ul.appendChild(
                    this.createItem("pager", next, options.nextText)
                );
            }
            ...
            if (options.firstLast) {
                ul.appendChild(
                    this.createItem("pager", pages, options.lastText)
                );
            }
        }
        return ul;
    ...
    truncate() {
        ...
        if (!this.instance.config.truncatePager) {
            for (let i = 1; i <= this.getPages(); i++) {
                pager.push(
                    this.createItem(i === currentPage ? "active" : "", i, i)
                );
            }
        } else {
            ...
            range.forEach(function (index) {
                if (lastIndex) {
                    if (index - lastIndex == 2) {
                        pager.push(
                            that.createItem("", lastIndex + 1, lastIndex + 1)
                        );
