...
@section('title')
    <title>Account Creation - Admin</title>
@endsection
        ...
        <div class="column box">
            <nav class="navbar" role="navigation" aria-label="main navigation">
                <div class="navbar-start">
                    <h1 class="title is-size-3 has-text-info">Create</h1>
                </div>
                <div class="navbar-end">
                    <div class="tags has-addons">
                        <a href="{{ route('admin.accounts.view', ['type' => 'admin']) }}"><button
        ...
        <section class="section" style="padding: 1.5rem;margin-top:-25px;">
            <div class="columns is-multiline is-12">
                ...
                <div class="column box is-8">
                    <form action="{{ route('admin.accounts.create') }}" method="POST">
                        @csrf
                        <div class="field">
                            <label class="label">Provide name</label>
                            <div class="control">
                                <div class="column is-6 no-padding">
                                    <input class="input is-rounded" id='department_name' name="name" type="text"
                                        placeholder="Provide a name for display" value="{{ old('name') }}">
                                </div>
                            </div>
                            @error('name')
                            ...
                            <div class="field">
                                <label class="label">Provide Email</label>
                                <div class="control">
                                    <div class="column is-6 no-padding">
                                        <input class="input is-rounded" id='department_name' name="name" type="email"
                                            placeholder="To be used for login" value="{{ old('name') }}">
                                    </div>
                                </div>
                                @error('name')
                                    <p class="help has-text-info">{{ $message }}</p>
